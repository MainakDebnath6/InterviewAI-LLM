...
function Step2Interview({ interviewData, onFinish }) {
  ...
  voices[0];
      ...
      if (preferredVoice?.name?.toLowerCase().includes("male")) {
        setVoiceGender("male");
      } else {
  ...
  const startMic = () => {
    if (!recognitionRef.current || isAIPlaying || !isMicOn) return;
  ...
  useEffect(() => {
    const askQuestion = async () => {
      if (isIntroPhase || !currentQuestion || !selectedVoice) return;
  ...
  useEffect(() => {
    if (isIntroPhase || !currentQuestion || isAIPlaying) return;
